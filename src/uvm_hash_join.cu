...
Result* uvm_hash_join(KeyValue *pHashTable, KeyValue *hashedDataSDev, int sizeS, int sizeR, int sizeResult, int &total)
{
    Result *result;
    int *count;
 
    if(cudaMallocManaged(&result, sizeof(Result) * sizeResult)==cudaSuccess){
        cout<<"success"<<endl;
    }
    else{
        cout<<"fail"<<endl;
    }
    // cudaMemAdvise(resultDev, sizeof(Result) * sizeResult, cudaMemAdviseSetAccessedBy, 0);
...
Result *normal_hash_join(KeyValue *pHashTable, KeyValue *hashedDataSDev, int sizeS, int sizeR, int sizeResult, int &total)
{
...
int main() {
    ...
    hash_join_util(dataR, hashedDataR,dir+"table_r2.csv");
    hash_join_util(dataS, hashedDataS,dir+"table_s2.csv");
    ...
    // unsigned long long reserve_memory = 3*(1UL<<30);
    // long long left_memory = ((24*(1UL<<30) - reserve_memory));
    // std::cout << "\nReserve memory: " << reserve_memory<<"\n";
    // std::cout << "\nLeft memory: " << left_memory/1024.0/1024.0 << "\n";
    // void* ptr{nullptr};
    // cudaMalloc(&ptr, reserve_memory);
    ...
    cudaMemcpy(&sizeResult, upperbound, sizeof(unsigned long long int), cudaMemcpyDeviceToHost);
    ...
    {
        cout<<"Executing normal hash join \n";
        result = normal_hash_join(pHashTable, hashedDataSDev,sizeS,sizeR,sizeResult,total);
    }
    else
    {
        cout<<"Executing UVM hash join \n";
        result = uvm_hash_join(pHashTable, hashedDataSDev,sizeS,sizeR,sizeResult,total);
    }
