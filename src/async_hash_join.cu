...
int main() {
    ...
    hash_join_util(dataR, hashedDataR,"table_r3.csv");
    hash_join_util(dataS, hashedDataS,"table_s3.csv");
    ...
    cudaMallocAsync(&hashedDataSDev, sizeof(KeyValue) * sizeS,stream);
    cudaMemcpyAsync(hashedDataSDev, hashedDataS.data(), sizeof(KeyValue) * sizeS, cudaMemcpyHostToDevice,stream);
