...
std::vector<KeyValue> generate_random_keyvalues(std::mt19937 &rnd,
                                                uint32_t numkvs) {
...
std::vector<KeyValue> shuffle_keyvalues(std::mt19937 &rnd,
                                        std::vector<KeyValue> kvs,
...
Time start_timer() { return std::chrono::high_resolution_clock::now(); }
...
int main() {
  // To recreate the same random numbers across runs of the program, set seed to
  // a specific number instead of a number from random_device
  ...
  while (true) {
    ...
    std::vector<KeyValue> insert_kvs =
        generate_random_keyvalues(rnd, kNumKeyValues);
    std::vector<KeyValue> delete_kvs =
        shuffle_keyvalues(rnd, insert_kvs, kNumKeyValues / 2);

    // Begin test
    printf(
        "Testing insertion/deletion of %d/%d elements into GPU hash table...\n",
    ...
    uint32_t num_inserts_per_batch =
        (uint32_t)insert_kvs.size() / num_insert_batches;
    for (uint32_t i = 0; i < num_insert_batches; i++) {
      insert_hashtable(pHashTable,
    ...
    uint32_t num_deletes_per_batch =
        (uint32_t)delete_kvs.size() / num_delete_batches;
    ...
    double milliseconds = get_elapsed_time(timer);
    double seconds = milliseconds / 1000.0f;
    printf("Total time (including memory copies, readback, etc): %f ms (%f "
           "million keys/second)\n",
           milliseconds, kNumKeyValues / seconds / 1000000.0f);
